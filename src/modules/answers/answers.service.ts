...
export class AnswersService {
	...
	async getAnswers(): Promise<Answer[]> {
		return await this.answerRepository.find({ relations: ['option','question','group','lesson'] });
	}
	...
	async updateAnswer(id: number, answerDto: UpdateAnswerDto): Promise<Answer> {
		...
		if (!answer) {
			throw new NotFoundException('The answer you want to update does not exist');
		}
		return this.answerRepository.save(answer);
	}
	...
	async deleteAnswer(id: number): Promise<void> {
		...
		if (!answer) {
			throw new NotFoundException('The answer you want to delete does not exist');
		}
